...
def load_model_from_ckpt(ckpt_path: str, device: torch.device) -> Tuple[LatentRendererModel, Dict[str, Any]]:
    ...
    model = LatentRendererModel(
        ...
        num_latents=num_latents,
        latent_dropout=latent_dropout,
        latent_noise_std=0.0,  # probe mode: no noise
    )
    model.load_state_dict(ckpt["model_state"], strict=True)
    model.to(device)
    model.eval()
...
def length_matched_indices(y_lang: torch.Tensor, y_len: torch.Tensor, seed: int = 42) -> torch.Tensor:
    """
    Build a subset where for each bucket, en and zh counts are matched:
      take n = min(count(en), count(zh)) per bucket
    ...
    for b in range(4):
        idx_en = torch.where((y_len == b) & (y_lang == 0))[0]
        idx_zh = torch.where((y_len == b) & (y_lang == 1))[0]
        n = min(int(idx_en.numel()), int(idx_zh.numel()))
        if n <= 0:
            continue
        perm_en = idx_en[torch.randperm(idx_en.numel(), generator=g)[:n]]
        perm_zh = idx_zh[torch.randperm(idx_zh.numel(), generator=g)[:n]]
        idx_all.append(perm_en)
        idx_all.append(perm_zh)

    if len(idx_all) == 0:
        return torch.arange(y_lang.size(0))
...
def train_probe_and_return_model(
    X_train: torch.Tensor,
    y_train: torch.Tensor,
    X_valid: torch.Tensor,
    y_valid: torch.Tensor,
    ...
    metrics = {"train_acc": float(tr_acc), "valid_acc": float(va_acc), "best_valid_acc": float(best)}
    return probe, metrics


@torch.no_grad()
def bucketwise_lang_acc(probe: nn.Module, X: torch.Tensor, y_lang: torch.Tensor, y_len: torch.Tensor) -> Dict[str, float]:
    ...
    for b in range(4):
        m = (yb == b)
        if int(m.sum().item()) == 0:
            out[f"bucket_{b}_acc"] = float("nan")
        else:
            out[f"bucket_{b}_acc"] = float((pred[m] == yl[m]).float().mean().item())
    return out
...
def main():
    ...
    # standard language probe
    print("\n=== Probe: Language ID (standard) ===")
    lang_probe_model, lang_res = train_probe_and_return_model(
        X_train=Xtr,
        y_train=ytr_lang,
        X_valid=Xva,
        y_valid=yva_lang,
    ...
    bw = bucketwise_lang_acc(lang_probe_model, Xva, yva_lang, yva_len)
    print({"lang_bucketwise_valid_acc": bw})

    # length-matched language probe
    idx_tr = length_matched_indices(ytr_lang, ytr_len, seed=1)
    idx_va = length_matched_indices(yva_lang, yva_len, seed=2)

    Xtr_m = Xtr[idx_tr]
    ytr_lang_m = ytr_lang[idx_tr]
    Xva_m = Xva[idx_va]
    yva_lang_m = yva_lang[idx_va]

    print("\n=== Probe: Language ID (length-matched) ===")
    _, lang_res_matched = train_probe_and_return_model(
        X_train=Xtr_m,
        y_train=ytr_lang_m,
        X_valid=Xva_m,
        y_valid=yva_lang_m,
    ...
    # length probe (4-way)
    print("\n=== Probe: Length bucket (4-way) ===")
    _, len_res = train_probe_and_return_model(
        X_train=Xtr,
        y_train=ytr_len,
        X_valid=Xva,
        y_valid=yva_len,
    ...
    out = {
        ...
        "len_probe": len_res,
        # new diagnostics
        "lang_probe_length_matched": lang_res_matched,
        "lang_bucketwise_valid_acc": bw,
        "length_matched_sizes": {
            "train": int(idx_tr.numel()),
            "valid": int(idx_va.numel()),
        },
    }
