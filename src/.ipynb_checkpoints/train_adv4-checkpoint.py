...
# @torch.no_grad()
# def eval_lang_acc(model, lang_clf, dl, device, max_batches: int = 200):
#     model.eval()
#     lang_clf.eval()
#     correct = 0
#     total = 0
...
#         out = model(en_ids, en_m, zh_ids, zh_m, labels)  # eval 下无噪声无 dropout
...
#     # ap.add_argument("--latent_noise_std", type=float, default=0.01)
#     # screened
#     ap.add_argument("--latent_dropout", type=float, default=0.00)
#     ap.add_argument("--latent_noise_std", type=float, default=0.00)
...
#                 diag = {"step": step, **eval_nll(model, valid_dl, device)}
#                 # added
#                 diag["lang_acc_valid_evalmode"] = eval_lang_acc(model, lang_clf, valid_dl, device)
#                 print("EVAL:", diag)
#                 with open(os.path.join(args.run_dir, "diag.jsonl"), "a", encoding="utf-8") as f:
#                     f.write(json.dumps(diag) + "\n")
#                 save_checkpoint(ckpt_path, model, lang_clf, opt_model, opt_lang, step, cfg)
