...
def main():
    ...
    # NEW: decoder-only switch
    ap.add_argument("--decoder_only", action="store_true", help="freeze encoder+bottleneck+shared, train decoder only")
    ap.add_argument("--resume_model_only", action="store_true", help="only load model weights from --resume, ignore optim/clf states")
    ...
    if args.decoder_only:
        # 1) freeze encoder path that determines zbar
        set_requires_grad(model.model.get_encoder(), False)
        set_requires_grad(model.bottleneck, False)
    
        # 2) IMPORTANT for mt5/t5: shared embedding is used by both encoder and decoder.
        #    If you keep it trainable, encoder input embedding changes -> zbar changes -> leakage can drift.
        if hasattr(model.model, "shared"):
            set_requires_grad(model.model.shared, False)
    
        # 3) enable decoder
        set_requires_grad(model.model.get_decoder(), True)
    
        # lm_head in T5 is often tied to shared; if tied, it will be frozen if shared is frozen.
        # That is okay for this stage, decoder blocks can still learn to be robust to noisy z.
        if hasattr(model.model, "lm_head"):
            # only set True if it's NOT tied to shared; safe default: leave it as-is
            pass
    ...
    # changed
    opt_model = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=args.lr_model)
    #
    ...
    if args.resume and args.resume_model_only:
        ...
        model.load_state_dict(ckpt["model_state"])
        step = int(ckpt.get("step", 0))
        print(f"[RESUME-MODEL-ONLY] loaded {args.resume}, step={step}")
    else:
        step = try_load_checkpoint(args.resume, model, lang_clf, len_clf, opt_model, opt_lang, opt_len, device)
