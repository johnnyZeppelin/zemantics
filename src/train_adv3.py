...
def main():
    ...
    ap.add_argument("--run_dir", default="runs/adv3_smoke")
    ...
    # NEW: mean and norm controls
    ap.add_argument("--lambda_norm", type=float, default=1.0, help="penalize (||z||-1)^2")
    ap.add_argument("--lambda_mean", type=float, default=1.0, help="penalize ||E[z]||^2")
    ap.add_argument("--lambda_mean_diff", type=float, default=1.0, help="penalize ||E[z_en]-E[z_zh]||^2")

    # adversary
    ap.add_argument("--lambda_langadv", type=float, default=0.5)
    ap.add_argument("--lang_steps", type=int, default=3)
    ap.add_argument("--adv_start_step", type=int, default=300, help="delay adversary until this global step")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_dl:
            ...
            # NEW: norm control to reduce dot-product domination
            norms = z_all.norm(dim=-1)
            loss_norm = ((norms - 1.0) ** 2).mean()

            # NEW: mean penalties to kill "global mean shortcut" and reduce language mean shift
            mu_all = z_all.mean(dim=0)
            mu_en = z_en.mean(dim=0)
            mu_zh = z_zh.mean(dim=0)
            loss_mean = (mu_all ** 2).mean()
            loss_mean_diff = ((mu_en - mu_zh) ** 2).mean()

            # adversary, delayed
            loss_lang_clf = torch.tensor(0.0, device=device)
            loss_lang_for_encoder = torch.tensor(0.0, device=device)
            acc_lang_clf = 0.0

            if step >= args.adv_start_step and args.lambda_langadv > 0:
                # 1) train classifier on detached features
                z_det = z_all.detach()
                for _ in range(args.lang_steps):
                    opt_lang.zero_grad(set_to_none=True)
                    logits_det = lang_clf(z_det)
                    loss_lang_clf = F.cross_entropy(logits_det, y_lang)
                    loss_lang_clf.backward()
                    opt_lang.step()

                with torch.no_grad():
                    acc_lang_clf = (logits_det.argmax(dim=1) == y_lang).float().mean().item()

                # 2) encoder adversary term (freeze clf params but allow grad through)
                for p in lang_clf.parameters():
                    p.requires_grad = False
                logits_adv = lang_clf(z_all)
                loss_lang_for_encoder = F.cross_entropy(logits_adv, y_lang)
                for p in lang_clf.parameters():
